...
import com.example.demo.subscriptionbackgroundflow.helper.logW
                    ...
                    when (billingResult.responseCode) {
                        BillingClient.BillingResponseCode.OK -> {
                            if (purchases != null) {
                                ...
                                Log.d(TAG, "onPurchasesUpdated: =>> Response OK But Purchase List Null Found")
                            }
                        }
                        else -> {
                            when (billingResult.responseCode) {
        ...
        if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
            for (product in purchase.products) {
                if (lifeTimeProductKeyList.contains(product)) {
                    ...
                    logI(tag = TAG, message = "handlePurchase: =>> productId -> $product")
                    BaseSharedPreferences(context).mIS_SUBSCRIBED = true
                }
            }
